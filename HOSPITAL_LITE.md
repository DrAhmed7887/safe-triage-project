...
### Native iOS app via Xcode (Capacitor wrapper)

For TestFlight / App Store distribution or when you want a "real" iOS
app icon that doesn't depend on Add-to-Home-Screen, the frontend is
also wrapped with [Capacitor](https://capacitorjs.com/) — the same
web build is bundled inside a native Xcode project.

**Mac, one-time bootstrap** (run from the repo root, ie. the
`safe-triage-project/` directory you got from `git clone`):

```bash
# 1. Get into the repo (the npm errors you saw earlier were because
#    you were still in ~/. The package.json lives inside the clone.)
git clone https://github.com/DrAhmed7887/safe-triage-project.git
cd safe-triage-project

# 2. Install JS deps and select Hospital Lite mode for the build.
cp frontend/.env.hospital_lite frontend/.env.local
npm --prefix frontend install

# 3. Scaffold the iOS project. Requires Xcode + CocoaPods on the Mac:
#    `xcode-select --install` and `sudo gem install cocoapods` if you
#    haven't already. This step generates `frontend/ios/App/App.xcworkspace`.
npm --prefix frontend run build
npm --prefix frontend run ios:add

# 4. Open in Xcode. Pick a development team / signing identity, plug in
#    an iPhone (or pick a simulator) and hit ▶︎.
npm --prefix frontend run ios:open
```

**After every web change** — re-bundle `dist/` into the iOS app:

```bash
npm --prefix frontend run ios:sync
# (re-open Xcode and run again, or `cap run ios` to do it from the CLI)
```

What this gives you over the PWA path:

- A real `SAFE-Triage.app` icon installed by iOS (not a Safari
  bookmark), with proper splash screen and standalone WebView chrome.
- A path to TestFlight + App Store via your Apple Developer account.
- The deterministic JS engine and the entire app shell ship **inside
  the IPA** — the app boots and triages with no network from the
  first launch (no "open online once first" caveat that the PWA
  flow has).
- Programmatic status-bar tinting and the Capacitor `App` lifecycle
  events wired up via `src/lib/nativeBridge.js`.

`capacitor.config.ts` lives at `frontend/capacitor.config.ts`. The
generated `frontend/ios/` Xcode project is committed; the synced web
build (`ios/App/App/public/`) and CocoaPods (`ios/App/Pods/`) are
